        ...
        <Teleport to="body">
            <div v-if="createModalIsOpen" class="bg-white fixed z-[999] w-[50rem] h-[30rem] left-[25%] top-[25%]">
                ...
                <form @submit="onSubmit" class="flex flex-col pl-[2rem]">
                    ...
                    <input v-model="guest.first_name" placeholder="first name" required class="border border-black w-[20rem]"/>
                    <label>Last name:</label>
                    <input v-model="guest.last_name" placeholder="last name" required class="border border-black w-[20rem]"/>
                    <label>Email:</label>
                    <input v-model="guest.email" placeholder="email" required type="email" class="border border-black w-[20rem]"/>
